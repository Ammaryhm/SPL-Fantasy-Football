...
.stTabs [data-baseweb="tab"]:not(:last-child)::after {
   content: "";
   position: absolute;
   right: -8px; /* Moves divider more to the right */
   top: 50%;
...
tabs = st.tabs(["🏠 Home", "🏆 League Table", "⚽ Fantasy Football", "📊 Statistics", "📅 Fixtures"])
st.markdown('</div>', unsafe_allow_html=True)
...
with tabs[2]:  # Fantasy Football Tab
    ...
    if "budget" not in st.session_state:
        st.session_state.budget = 100_000_000  # 100M SAR budget
    ...
    with fantasy_tabs[0]:
        ...
        with col1:
            if len(st.session_state.fantasy_team) == 0:
                ...
                if st.button("🎲 Add My Starting 11", type="primary"):
                    # Predefined player names for controlled prototype
                    predefined_names = [
                    ...
                    # Find these specific players in the database
                    selected_players = []
                    ...
                    # Reset team and budget
                    st.session_state.fantasy_team = []
                    st.session_state.budget = 100_000_000
                    
                    # Add the predefined players to the team
                    for player in selected_players:
                        ...
                        st.session_state.fantasy_team.append(player_dict)
                        st.session_state.budget -= player["price"]
                    
                    st.success(f"✅ Added {len(selected_players)} players to your squad!")
                    st.rerun()
                ...
                if len(st.session_state.fantasy_team) > 0:
                    ...
                    st.markdown("---")
                    st.markdown(f"**Showing 15 of your players**")
        ...
        with col2:
            ...
            st.markdown("---")
            if team_size == 11:
                avg_overall = sum(p.get("overall", 60) for p in st.session_state.fantasy_team) / 11
                st.markdown(f"#### 👥 Current Squad (11/11)")
            else:
                st.markdown(f"#### 👥 Current Squad ({team_size}/11)")

            if team_size > 0:
                # 🧢 Captain / Vice Captain Dropdowns
...
with fantasy_tabs[1]:
    ...
    if len(st.session_state.fantasy_team) == 11:
        ...
        # Create player lookup from your fantasy team and all available players
        ...
        def display_player_minimal(player, index):
            ...
            # Ensure overall is displayed properly
            if ovr == "-" or ovr is None:
                ovr = 60
            
            kit_number = f"{index:02}"  # Pads 1 → 01, 9 → 09, 10 → 10
        ...
        # Show match prediction based on averages
        ...
        with col_stats2:
            if opponent_xi:
                ...
                st.markdown("### 🔴 Generate Opponent XI")
                st.info("Generate opponent XI to see their stats")
        ...
        if len(user_xi) == 11 and opponent_xi:
            ...
            # Calculate opponent team average
            for player in opponent_xi:
                rating = player.get("overall", 60)
                try:
                    rating = float(rating) if rating != "-" else 60.0
            ...
            if user_ratings and opponent_ratings:
                ...
                # Add a button to regenerate opponent team (limited to 3 times)
        ...
        # Add spacing from the prediction box
        st.markdown("<div style='margin-top: 1.5rem;'></div>", unsafe_allow_html=True)

        if remaining_regenerations > 0:
            # Create column layout: empty space on the left, button on the right
            col1, col2, col3 = st.columns([6, 1, 1])  # adjust ratios as needed
            with col3:
    ...
    if len(user_xi) == 11 and opponent_xi:
        simulate_match(user_xi, opponent_xi)
